...
**Status: Implemented (2026-07-04, code + docs; verified live on a scratch DB).**
Added a **JARVIS personality** layer (`server/lib/brain/persona.js` +
`prompts/persona.md`): one toggle (`app_settings` `jarvis_persona` → env
`JARVIS_PERSONA` → default on) that prepends the persona system block to the
brain's assistant/briefing calls (`applyToSystem`) and provides a deterministic
copy switch (`line(plain, jarvis)`) for no-model text (nudge/push copy, the
no-provider briefing fallback) - dry, formal-but-warm butler voice that addresses
the user as "sir", with hard honesty rules that outrank the voice. **Briefings**
(`server/lib/briefings.js`, new additive `briefings` table): deterministic
context assembly from project pulse (G2) + GitHub overview (I) + dashboard-run
activity since midnight + waiting agents, composed to prose by the brain
(standard tier, persona-wrapped) when a provider is configured, else a
deterministic composition of the same facts (nothing invented). `runBriefing()`
persists the row, files a `source: briefing` note (G1), fires a `briefings` push,
broadcasts `briefing_created`, and returns a Siri-readable `speech` variant. A
single 60s `tick()` on the **shared** Phase-L scheduler fires morning/evening
once per day per kind (persisted last-fired stamp survives restarts; a catch-up
beyond a 3h grace window is skipped so a long-asleep Mac never delivers a morning
briefing at night). **Deterministic nudges** (`server/lib/nudges.js`): a
run-status subscriber pushes `run_completions` on a terminal `error`; a 60s
waiting-agent sweep pushes `waiting_agents` past a threshold, deduped to once per
waiting spell; neglected projects are briefing-mentioned, not instant-pushed -
all gated by the C3 category toggles. Route `server/routes/briefings.js`
(`/api/briefings` - history, `POST /run`, `GET/PUT /config` for times + nudges +
persona) behind the Run router's loopback-Origin guard; the "morning briefing"
voice intent added to `server/lib/assistant.js`. Client: a new **Briefings** page
(`client/src/pages/Briefings.tsx`, route `/briefings` + sidebar `nav:briefings`
en/tr/zh) lists briefings, runs one on demand, and edits the Phase-J config;
`api.briefings` group + `Briefing`/`ProactiveConfig` types + a `briefing_created`
WS union member. Recurring tasks + `nudges.start` wired in `server/index.js`.
Tests: new `server/__tests__/briefings.test.js` (12 tests: persona
enable/disable, deterministic-fallback composition with/without persona,
runBriefing persist+note+speech, the REST surface + config round-trip incl.
persona persistence affecting composed copy, waiting-agent dedup + prune +
disable gating, run-failed status gating, and the voice-intent matcher) -
`npm run test:server` now **662 pass** (was 650). Client `tsc -b` clean,
`npm run test:client` **248 pass, zero snapshot churn** (Briefings isn't in
`screens.snapshot.test.tsx`, and Dashboard/Settings were deliberately untouched
- Phase-J config lives on the Briefings page, the same contextual-config choice
Notes/Skills/GitHub made). Docs: README (feature row + route table),
ARCHITECTURE (prose subsection + routing-table row), docs/API.md (REST + the
`briefing_created` WS event), docs/DATABASE.md (`briefings` table), SETUP.md
(Phase-J section + updated voice-intent table), `.env.example` (`JARVIS_PERSONA`).
**Deviations / not done**: (1) the briefing is a first-class server feature
(scheduled + voice + REST + page) rather than literally "just a skill" - the seed
`docs/skills/daily-briefing.md` still works as a simple generic-brain-step
alternative, but the richer context-assembled briefing needed data the generic
`brain` step doesn't carry; this is a deliberate, documented divergence from the
"briefing = just a skill" framing. (2) **System health** briefing input is absent
because Phase I step 2 (Mac/work-PC stats) was never built; likewise **calendar/
email agenda** (Phase I step 3) - both are omitted gracefully from context
assembly and slot in with no composer change once those land. (3) On-device
verification deferred (typecheck + both full suites + a live scratch-DB compose
against the user's real Gemini key are the gate here): pushes received on a
backgrounded phone over cellular, Siri reading a scheduled briefing in the car,
and a real morning/evening tick crossing its wall-clock time. VN/CN READMEs +
`index.html` + wiki intentionally not mirrored (English core, matching Phases
D/E/F/G/H/I/K/L).
