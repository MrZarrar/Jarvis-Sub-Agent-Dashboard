...
window.__WIKI_CONTENT_I18N = {
  zh: {
    ...
    "Spawn <code>claude</code> subprocesses straight from the dashboard with a chat-style streaming UI — multi-turn <b>Conversation</b> or single-shot <b>Headless</b> mode. One-click <b>Resume</b> on any past conversation spawns <code>claude --resume</code> seeded with the prior transcript. Re- attach reconciles in-memory logs with the on-disk JSONL so navigating away never loses history. Slash-command autocomplete, file references, live token/context-window meter, and a thinking-effort dial bring TUI parity to the browser. Same-origin guard blocks drive-by spawns. An optional interactive permission gate pauses every tool call for an in-page Allow/Deny, pushing a notification with a deep link when you're not watching the tab.":
      "直接从仪表盘以聊天式流式 UI 生成 <code>claude</code> 子进程——可选多轮的 <b>Conversation</b> 或单次的 <b>Headless</b> 模式。对任意过往对话一键 <b>Resume</b>，会以先前的 transcript 作为种子生成 <code>claude --resume</code>。重新接入会将内存中的日志与磁盘上的 JSONL 进行协调，因此离开页面也绝不会丢失历史。斜杠命令自动补全、文件引用、实时的 token/上下文窗口计量表以及思考强度旋钮，让浏览器具备与 TUI 同等的体验。同源保护可阻止顺带发起的恶意生成。可选的交互式权限门会在每次工具调用时暂停，等待页面内的允许/拒绝操作；如果你没有盯着页面，还会推送带深链接的通知。",
