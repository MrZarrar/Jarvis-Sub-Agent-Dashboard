...
function fail(step, res, body) {
  return {
    ...
    error: {
      ...
      detail:
        (body && (body.error_description || body.message || body.errors || body.raw)) || null,
    },
  };
}
...
async function ebaySearch({ q, limit = 10 }, fetchImpl = fetchDefault) {
  ...
  return { ok: true, total: body.total || items.length, items, note: "active listings (asking prices), not sold comps" };
}
...
async function ebayCreateDraftListing(input, fetchImpl = fetchDefault) {
  const cfg = config.getConfig().ebay;
  const { sku, title, description, price, quantity = 1, condition = "USED_EXCELLENT", imageUrls = [] } =
    input || {};
  if (!sku || !title || !price) {
...
async function keepaTokenStatus(fetchImpl = fetchDefault) {
  ...
  const res = await fetchImpl(
    `https://api.keepa.com/token?key=${encodeURIComponent(cfg.apiKey)}`
  );
  const body = await jsonOrText(res);
...
async function testConnection(provider, fetchImpl = fetchDefault) {
  switch (provider) {
    case "ebay": {
      ...
      const cfg = config.getConfig().ebay;
      if (!cfg.refreshToken) return { ok: true, detail: "app token OK; no sell-side refresh token yet" };
