...
// Business mode (Phase BM): business todos live in notes tagged `business`;
// dev mode (the default) excludes those notes, so the two boards never mix.
const BUSINESS_TAG = "business";

function hasBusinessTag(meta) {
  return (
    Array.isArray(meta.tags) && meta.tags.some((t) => String(t).toLowerCase() === BUSINESS_TAG)
  );
}
...
/** The whole board in one shape. Never throws. `mode: "business"` swaps the
 *  todo lane to business-tagged notes and blanks the Monday lanes (a dev
 *  surface); schedules/agents/runs stay mode-neutral. */
 ...
 *  (title = local YYYY-MM-DD; business mode uses its own `YYYY-MM-DD Business`
 *  note tagged `business`), creating the note on first add. */
function addTodo({ text, mode }) {
  ...
  if (!note) {
    const tags = business ? ["daily", BUSINESS_TAG] : ["daily"];
    note = notes.createNote({ title, body: todoLine, tags });
