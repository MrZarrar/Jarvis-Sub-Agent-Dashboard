...
| `GET`    | `/api/run/:id/permissions`            | List every interactive permission request for a run (pending + resolved), sorted by `openedAt` |
| `POST`   | `/api/run/:id/permission/request`     | Opened by `scripts/permission-gate.js` (a PreToolUse hook), not the UI. Body: `{ toolUseId, toolName?, toolInput? }`. Idempotent per `toolUseId`; `409 ENOTINTERACTIVE` if the run never opted in |
