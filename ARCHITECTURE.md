...
Pure composition over the earlier phases: Jarvis reaches out first.

- **Persona (`server/lib/brain/persona.js` + `prompts/persona.md`).** One toggle
  (`app_settings` `jarvis_persona` → env `JARVIS_PERSONA` → default on) decides
...
- **Briefings (`server/lib/briefings.js`, `briefings` table).** `assembleContext()`
  ...
  crosses its configured time - once per day per kind (a persisted last-fired
  stamp guards restarts; a catch-up beyond a 3h grace window is skipped so a
  long-down server never delivers a morning briefing at night).
- **Nudges (`server/lib/nudges.js`).** Deterministic, never model-composed: a
  run-status subscriber pushes `run_completions` on a terminal `error`; a 60s
  waiting-agent sweep pushes `waiting_agents` for an agent stuck waiting past the
  threshold, deduped to once per waiting spell (in-memory, re-armed when the
  agent stops waiting). Neglected projects are surfaced in briefings, not
  instant-pushed. Everything is gated by the C3 push-category toggles.
- **Routes (`server/routes/briefings.js`, `/api/briefings`).** History + on-demand
  `POST /run` + the combined `GET/PUT /config` (briefing times, nudge rules,
  persona), behind the loopback-Origin guard (composing writes notes + fires
  pushes). The "morning briefing" voice intent (`server/lib/assistant.js`) calls
  `runBriefing` and reads back its `speech`.
- **Client.** A `/briefings` page lists briefings, runs one on demand, and edits
  the Phase-J config (times, nudges, persona toggle); it live-updates on
  `briefing_created`. Config lives on the page (not Settings) to avoid
  Settings-snapshot churn - the same contextual-config choice Notes/Skills/GitHub
  made.
- **Startup.** `server/index.js` calls `nudges.start({ runs })` and registers the
...
| --------------- | ------------- | ------------------------------------------------------ |
| `/`             | Dashboard     | Command bridge (JarvisCore + graphical instrument cluster: sessions sparkline, agents/subagents orbit, events/min sparkline, cost dial) plus one Operations panel with an internal Operations/Health segmented control (was page-level tabs). Operations: `GET /api/stats` (incl. `session_window` for the core's countdown ring), `GET /api/agents`, `GET /api/events`, `GET /api/agents?session_id={sid}` (subagent hierarchy), `GET /api/analytics` (7-day session-count trend), `GET /api/pricing/cost` (`daily_costs` powers the cost dial's today-vs-week reference), dynamic item counts via `ResizeObserver`. Health: `GET /api/settings/info` + `GET /api/workflows` (5 s auto-refresh) - composite health score, storage donut, cache/error/success gauges, tool invocation bars, subagent effectiveness, model token distribution, compaction stats |
...
| `/wall`         | Wall          | TV wall mode (Phase U). Chrome-free, rendered **outside Layout** (no sidebar/Tabby/tab bar), **read-only by contract** (zero mutating affordances, since it may run on a shared screen), and fixed to one viewport (`h-screen overflow-hidden` - never scrolls). Two views, toggled by a top-right button (viewing chrome, not data): **Bridge** (default) - big-type clock, the needs-you strip (readonly), a free-floating holographic bridge (no panel chrome, soft radial glows: `JarvisCore` hologram center stage - live count, 5h-window countdown ring, USED % - flanked by a spinning active-agents cluster and the vault "brain", the knowledge graph as a slowly rotating multi-colored constellation, nodes hued by type from the chart palette, links redraw like synapses firing, self-hides when the vault is empty), then an auto-cycling secondary band on a diffused translucent pane (`?panels=ops,todo,monday,github` by default; the GitHub view renders ring gauges and only joins the rotation once GitHub is configured). The Ops Room isn't in that rotation - it has its own view. **Room view** - centers the clock in its own header, and the Ops Room (`AgentRoom`, `size="wall"`, fit-contains on width AND height so the whole room stays visible with no crop) becomes the hero filling most of the screen, flanked by the Jarvis hologram (scaled to a 224px thumbnail) and the vault brain (`compact` prop, smaller graph, no caption) as small side indicators, vertically centered against the room. Full HUD personality in both views: `hudMode.init()`, typed incantations ("ultron"/"jarvis"), `?hud=` deep link, live error-storm/swarm triggers, and the UltronTakeover glitch overlay. Reuses `GET /api/stats`, `GET /api/events`, `GET /api/vault/graph`, `GET /api/agents`, `GET /api/github`; WS-live via the shared event bus with a 30s poll fallback; requests a screen wake lock where supported; any tap/click toggles browser fullscreen (gesture-gated by the Fullscreen API - kiosk launches with `--kiosk`/`--start-fullscreen` never need it) |
...
| **Delivery categories** | The optional `category` arg tags a send with one of `permission_requests` / `run_completions` / `waiting_agents` / `briefings` / `account_swaps` / `scheduled_prompts` / `skills` / `github` / `finance` / `monday` (`PUSH_CATEGORIES` in `lib/push.js`). Before dispatching, `sendPushToAll` calls `isCategoryEnabled(db, category)` against the `notification_prefs` table; a muted category short-circuits the whole dispatch (native + web push) to a no-op so a producer can't spam a category the user turned off. A missing row, absent category, or DB read error all **fail open** (deliver), so a permission request is never silently swallowed. State is read/written via `GET /api/push/categories` and `PUT /api/push/categories/:category`, surfaced as per-device-independent switches under **Settings → Notifications**. |
| **Notification inbox (Phase O)** | Producers no longer call `sendPushToAll` directly — every notification routes through the facade `server/lib/notify.js` → `notify({category, title, body, url, data, source, dedupeKey, escalate})`, which persists a row in the `notifications` table, broadcasts `notification_created` on the WS, and then delegates the push leg to `sendPushToAll`. A muted category is a *full* no-op (no push, no row). A `dedupeKey` coalesces same-category repeats into the existing **unread** row (re-pushing only when `escalate` is set). The inbox surfaces on the Tabby ball: an unread badge, an arrival nudge bubble (click → popup opens on the Inbox tab with the item focused), and an Inbox tab in the popup (tap → deep link + mark read; read state syncs across devices via `notification_read`). REST: `GET /api/notifications?unread=&limit=`, `POST /api/notifications/:id/read`, `POST /api/notifications/read-all`. Copy rules (exact names, counts, times, deep links) and the producer table live in `docs/NOTIFICATIONS.md`. |
...
| **`prefs.ts`** | `localStorage`-backed preferences (enabled / muted / manual-sleep / position / expanded / provider / **panel drag offset** / **conversation transcript**) with cross-tab sync for the enabled/muted set. |
...
**File/shell access is empty by default.** `read_file`/`write_file`/`list_dir`/`shell` only reach paths inside a Settings-managed allowlist (`app_settings.assistant_allowed_roots`, a JSON array). With no roots granted, all file access errors out - "given access" is literal. **Settings → Assistant Access** is the UI for this allowlist: an add/remove list backed by `GET`/`PUT /api/settings/assistant-roots` (previously API-only; the client card was a documented gap, now closed).

**The `claude_agent` delegate (full agent, opt-in).** A text-only brain tier (Gemini) has no internet and no multi-step agency of its own. The `claude_agent` action closes that gap without adding web/fetch plumbing to the server: it hands the task to a headless `claude -p` agent (`providers/claude.js` → `runAgentTask`, spawned with `--permission-mode bypassPermissions` so it can actually use its tools unattended), which brings **live web access (WebSearch/WebFetch), the installed `agent-reach` skill** (Twitter/Reddit/YouTube/GitHub/xiaohongshu/Bilibili/RSS/arbitrary URLs), and file/shell tools - same local `claude` binary + OAuth as everywhere else, no API key. Gemini's function-calling loop invokes it whenever an answer needs the internet or work it can't do itself. Its `risk` is **dynamic**, driven by `app_settings.assistant_autonomy` (a getter on the action, read by the one dispatcher): `off` (default) → the tool is not offered to the model and `execute` refuses; `ask` → `confirm` (Gemini proposes it, the human taps once in the popup); `auto` → `safe` (fires inline with no tap, and non-interactive sources may fire it too). The user sets the level in **Settings → Assistant Access** ("Full agent"). This is the only place a `safe`-risk action can reach the shell - by explicit, user-owned opt-in, never silently.

**The `open_browser` action (computer use, Phase Z Tier 0).** For "open it in my browser" / when the user is at the Mac and wants to actually click and scroll: `open_browser` (`{query?, url?}`) runs `open <url>` and opens the URL/search in the user's **real** default browser - their logged-in session, fully interactive, no bot/CAPTCHA checks. It opens on the Mac itself and streams nothing to the dashboard (so it's a Mac-local move, not a phone one). macOS-only; gated by the same `assistant_browse_safe` opt-in as `browse`. ~1 shell call - the lazy, correct path for interactive browsing, complementing Tier 1's remote read-only view.

**The `browse` action (computer use, Phase Z Tier 1).** "Search this and show me" opens a **headless Chromium** (Playwright) on the Mac and streams a screenshot of every step to the dashboard over the existing WebSocket as a `browse_frame` message - so the user watches from their **phone**, not just at the Mac the browser runs on. `server/lib/browser.js` owns a *single global session* (one page, launched lazily, auto-closed after ~2 min idle - `ponytail: pool later if throughput matters`); Playwright is a lazy `require`, so if it (or its Chromium) isn't installed the action degrades to an honest error instead of crashing at boot. The `browse` action (`{query?, url?}`) navigates and returns `{url, title, view:"/browse"}`; its `risk` is **dynamic** like `claude_agent`, driven by `app_settings.assistant_browse_safe` (a getter read by the one dispatcher): `false` (default) → `confirm` (one tap in the popup / Browse page); `true` → `safe` (read-only navigation fires inline, so Tabby/Siri can drive it). The user opts in at **Settings → Assistant Access** ("Browse without asking"). The `/browse` page (`client/src/pages/Browse.tsx`) renders the frame stream (URL bar + narration) and is mobile-friendly.

**The `computer_use` action (computer use, Phase Z Tier 2 - the real desktop).** Stronger than Tier 1: this drives the user's **actual Mac desktop**, not a headless browser. `server/lib/computer-use.js` has no session/process to manage (`ponytail:` each action is a one-shot CLI spawn, so there's no browser.js-style lazy-launch/idle-close lifecycle to replicate) - it shells out to `screencapture` for screenshots and macOS **"System Events" UI scripting** (`osascript`) for a single left-click at a coordinate, literal keystrokes, and named keypresses (return/tab/escape/arrows/etc.) - the same native-platform-over-dependency pattern as Phase Y's osascript-driven Messages/Mail. The `computer_use` action (`{steps?}`, up to 10 of `{type:"click",x,y}` / `{type:"type",text}` / `{type:"key",key}` / `{type:"screenshot"}`; no steps = just a screenshot) runs each step and screenshots after it, streaming every frame to the dashboard as a `computer_use_frame` WS message - so the live view works from the phone too. Because it actually moves the mouse and types on the real machine, it is gated by its **own** dynamic-risk opt-in (not `browse`'s - opting one in does not opt in the other): `app_settings.assistant_computer_use_safe`, `false` (default) → `confirm`; `true` → `safe`. The user opts in at **Settings → Assistant Access** ("Control my screen without asking"). The `/computer-use` page (`client/src/pages/ComputerUse.tsx`) renders the frame stream; unlike `/browse` there's no URL bar (you need to see the screen before deciding where to click), just a manual "take a screenshot" trigger plus whatever Jarvis drives via the gated action. Requires macOS **Accessibility** permission (System Events UI scripting) and **Screen Recording** permission (`screencapture`) granted to the server process - without them, `osascript`/`screencapture` fail with a clear stderr message that surfaces through the dispatcher's normal error path. **Deferred:** a fully autonomous vision loop (Jarvis viewing every screenshot and picking the next click on its own, with no pre-specified steps) needs either verified `claude -p` image input or a provider's tool-result image feedback, both unverified elsewhere in this plan - not built here; the primitive (a bounded, LLM-specified step sequence per turn) is the honest, solid piece that ships.
...
**Provider-failure honesty (bugfix).** `assistant-actions/index.js`'s `respond()` previously fell back to the tiered brain router on ANY failure of the requested provider - including a `providerX` request quota/API error - and returned only the fallback's `provider`, silently relabelling the reply as if the fallback had been requested all along (it even repinned the popup's provider picker to match). It now additionally returns `requestedProvider` + `providerError` when a fallback occurred; `POST /api/assistant/ask`'s response and `TabbyPanel.tsx` both surface these additively - the popup renders an explicit "X failed (reason) — answered via Y" notice instead of pretending nothing happened, and no longer silently repins the picker on an involuntary fallback (only on a deliberate spoken directive).

### API + schema additions (all additive)

- `POST /api/assistant/ask` request gains optional `provider` and `context` (`{page, runId?, hudMode?}`); response gains `actions[]` (`{name, params, status:"done"|"needs_confirm", confirmToken?}`), `provider` (the tier that answered), and - only on a fallback - `requestedProvider` + `providerError`. Old callers (Siri Shortcuts) are unaffected.
...
- `GET`/`PUT /api/settings/assistant-roots` - manage the file-access allowlist (now with a Settings UI, see above).
- `GET`/`PUT /api/settings/assistant-autonomy` - the `claude_agent` delegate level (`off`|`ask`|`auto`), stored in `app_settings.assistant_autonomy`. Settings UI at Assistant Access → "Full agent".
- `GET`/`PUT /api/settings/browse-safe` - opt read-only `browse` into risk `safe` (`{safe}` boolean), stored in `app_settings.assistant_browse_safe`. Settings UI at Assistant Access → "Browse without asking".
- `browse_frame` WebSocket message (Phase Z, backward-compatible addition) - `{url, title, image (jpeg data URL), note, at}`, one per browse step; consumed by the `/browse` page.
- `GET`/`PUT /api/settings/computer-use-safe` - opt `computer_use` into risk `safe` (`{safe}` boolean, own key, independent of `browse-safe`), stored in `app_settings.assistant_computer_use_safe`. Settings UI at Assistant Access → "Control my screen without asking".
- `computer_use_frame` WebSocket message (Phase Z Tier 2, backward-compatible addition) - `{image (jpeg data URL), note, at}`, one per computer-use step; consumed by the `/computer-use` page. `GET /api/assistant/computer-use/last` hydrates the page on mount (mirrors `/browse/last`).
