...
**Supported languages:** English (`en`), Chinese (`zh`), Turkish (`tr`)
...
- `fallbackLng`: `"en"`
- `nonExplicitSupportedLngs`: `true` (e.g. `tr-TR` resolves to `tr`)
...
erDiagram
    ...
    LANGUAGE {
        string code "en|zh|tr"
        string locale "en-US|zh-CN|tr-TR"
    }
    NAMESPACE {
...
- Keep key parity across `en`, `zh`, `tr` files for the same namespace.
...
sequenceDiagram
    ...
    U->>SB: Click language button (EN/ZH/TR)
    SB->>I: changeLanguage("tr")
...
stateDiagram-v2
    ...
    Detecting --> Loaded_tr: localStorage/navigator resolves tr
    Detecting --> Loaded_en: unsupported locale -> fallback en

    Loaded_en --> Loaded_zh: user switches to zh
    Loaded_en --> Loaded_tr: user switches to tr
    Loaded_zh --> Loaded_en: user switches to en
    Loaded_zh --> Loaded_tr: user switches to tr
    Loaded_tr --> Loaded_en: user switches to en
    Loaded_tr --> Loaded_zh: user switches to zh
```
...
|---|---|---|
| Resource parity | Same key coverage across `en/zh/tr` | Missing key detection in CI |
| Locale fallback | Unknown locales fall back to `en` | `tr-TR` resolves to `tr` |
| Terminology consistency | Canonical terms stay stable | `Agent`/`Subagent` expectations |
| Date/number formatting | Locale-specific output shape | `zh-CN`, `tr-TR` formatting |
...
| Unexpected fallback to English | Unsupported locale code | Ensure code normalizes to `en|zh|tr` and key exists in target namespace |
...
gantt
    ...
    Fill en/zh/tr namespace files   :a2, after a1, 5d
