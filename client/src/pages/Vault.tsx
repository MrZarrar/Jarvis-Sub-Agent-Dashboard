 ...
 * drag-orbit / zoom-to-core, click selects (camera flies in), double-click
 * focuses the 2-hop neighborhood (?focus=<id> deep link). This page owns the
 * data + chrome: load/WS refresh, type filters, search dim, the node detail
 * panel, entity-engine controls and fx diffing, and the Recall panel - old
 * but connected notes resurfaced as active-recall questions (gold in the
...
export function Vault() {
  // Engine fx state lives in one stable object the sphere's draw loop reads;
  // node/edge diffing below (not the WS payload) decides what animates.
  ...
  // ── Visible subset (type filter + focus neighborhood) ─────────────────────
  const visible = useMemo(() => {
    if (!graph) return { nodes: [] as VaultGraph["nodes"], edges: [] as VaultGraph["edges"] };
    let nodes = graph.nodes
      .map((n) => ({ ...n, type: normalizeType(n.type) }))
  ...
  // Neural fx diffing: while the engine runs (or just ran), anything NEW in a
  // refetch fires - fresh nodes are born, fresh edges grow in hot.
  useEffect(() => {
    const fx = fxRef.current;
    const now = performance.now();
    ...
    for (const [key, t0] of fx.hotEdges) if (now - t0 > 5000) fx.hotEdges.delete(key);
  }, [visible]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      api.notes
        .list({ q })
        .then((res) => {
          if (active) setBodyMatchIds(new Set(res.items.map((note) => note.id)));
        })
        .catch(() => {
          if (active) setBodyMatchIds(new Set());
        });
    }, 150);
    return () => {
      active = false;
  ...
  const matchIds = useMemo(() => {
    ...
    return new Set(
      visible.nodes
        .filter((n) => bodyMatchIds.has(n.id) || n.title.toLowerCase().includes(q))
        .map((n) => n.id)
    );
  }, [bodyMatchIds, query, visible]);
  ...
  const recallDone = useCallback((id: string) => {
    ...
    setRecall((r) => r.filter((x) => x.id !== id));
  }, []);
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex flex-col">
      ...
      <div className="relative flex-1 min-h-0 mx-4 mb-4 card overflow-hidden">
        ...
        {empty && (
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-gray-500 gap-2 px-6 pointer-events-none">
        ...
        {/* Recall: resurfaced notes as questions - gold nodes in the globe. */}
        {recall.length > 0 && (
          <div className="absolute bottom-3 left-3 z-10 w-72 max-w-[85%] rounded-xl border border-amber-400/25 bg-surface-1/90 backdrop-blur-sm">
            <div className="flex items-center justify-between px-3 py-2 border-b border-amber-400/15">
              <p className="text-[11px] uppercase tracking-wider text-amber-300/90 flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5" /> Recall
              </p>
              <button
                type="button"
                onClick={() => setRecall([])}
            ...
            <ul className="p-2 space-y-1.5 max-h-56 overflow-y-auto">
              {recall.map((r) => (
                <li key={r.id} className="rounded-lg bg-surface-2/70 px-2.5 py-2">
                  <p className="text-xs text-gray-200 leading-snug">{r.question}</p>
                  <div className="mt-1.5 flex items-center gap-2">
                    ...
                    >
                      <Crosshair className="w-3 h-3" /> {r.title.slice(0, 24)}
                    </button>
                    <button
                      type="button"
                      onClick={() => recallDone(r.id)}
                      className="ml-auto text-[11px] text-gray-500 hover:text-amber-300 flex items-center gap-1"
                      title="Reviewed - rest it for a few days"
                    >
