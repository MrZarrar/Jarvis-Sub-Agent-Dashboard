...
function StepPill({ label }: { label: string }) {
  return (
    <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-cyan-500/15 text-cyan-300 border border-cyan-500/20 whitespace-nowrap">
      {label}
    </span>
  );
}
...
function PatternDetail({ pattern }: { pattern: WorkflowPattern }) {
  ...
  return (
    <div className="border-t border-cyan-500/20 bg-surface-1/40 px-4 py-3.5 space-y-3.5">
        ...
        <div className="flex items-center flex-wrap gap-1.5">
          {pattern.steps.map((step, i) => (
            <span key={i} className="flex items-center gap-1.5">
              <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-cyan-500/15 text-cyan-200 border border-cyan-500/25">
                <span className="text-cyan-400/70 mr-1.5 text-[10px] font-bold">{i + 1}</span>
                {step}
              </span>
      ...
      {/* Suggestion */}
      <div className="bg-cyan-500/5 border border-cyan-500/15 rounded-md px-3 py-2.5">
        <p className="text-[10px] font-semibold text-cyan-300 uppercase tracking-wider mb-1 flex items-center gap-1.5">
