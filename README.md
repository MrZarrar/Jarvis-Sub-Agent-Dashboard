...
- [Mini JARVIS - Floating Companion](#mini-jarvis--floating-companion)
...
The UI ships with built-in locale switching for English (`en`), Chinese (`zh`), and Turkish (`tr`). Language resources are loaded by namespace and persisted through browser storage for stable user preference across refreshes.

```mermaid
flowchart LR
    A["User selects language<br/>en / zh / tr"] --> B["i18next detector<br/>localStorage -> navigator"]
...
| Feature                            | Description                                                                                                                                                                                                                                                                  |
|------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Dashboard**                      | Graphical instrument cluster around the central JarvisCore: total-sessions sparkline (7-day trend), agents/subagents orbiting-dot count (working = animated, waiting = amber pulse), live events/min sparkline, and a cost dial (today vs. this week's daily average) - the core itself carries a depleting countdown ring plus ticking digits for the session-usage window reset. Below it, one **Operations** panel holds an internal segmented control persisted in `localStorage`: **Operations** - active agent cards with collapsible subagent hierarchy and a recent activity feed, dynamic item counts filling available height via `ResizeObserver`. **Health** - composite system health score ring (weighted: 0.4 × success rate + 0.25 × cache hit rate + 0.25 × (100 − error rate) + 0.1 × (100 − heap %)), storage engine donut chart with record distribution, cache performance / error rate / success rate gauges, tool invocation horizontal bar chart (top 8), subagent effectiveness bars, model token distribution, and compaction impact stats, auto-refreshing every 5 s from `/api/settings/info` and `/api/workflows`. Cursor-following tooltips with viewport edge detection on every chart. On mobile the instrument cluster collapses to a 2×2 grid under the core, and the app shell swaps the hamburger drawer for a bottom tab bar (Home / Agent Board / Sessions / Activity / More) |
...
| **Run Claude**                     | Spawn `claude` subprocesses directly from the dashboard with a chat-style streaming UI. Two modes: **Conversation** (multi-turn - stdin stays open, follow-up turns are piped as stream-json envelopes) and **One-shot** (headless, single prompt → single response). Conversation mode also supports **resuming any existing session** via `claude --resume <id>` - pick from your full sessions history with a searchable picker. The unified active-runs / history modal also offers two zero-config jump buttons: **Resume** on any past conversation row spawns `claude --resume <id>` immediately and seeds the chat with the prior transcript so you land in the live view with full context (no need to retype a prompt - the spawn idles on stdin until you send a follow-up); **View** on any past one-shot row loads the captured transcript inline into the run viewer as read-only (no spawn - same panel, no Stop/follow-up controls). Active runs switcher in the header lets you leave a run in the background, start another, and re-attach later. Re-attach is durable: the client reconciles the spawner's in-memory envelope log (`?envelopes=1`) with the session's on-disk JSONL transcript and prefers whichever has more user/assistant messages, so navigating away from a resumed run and coming back keeps the full prior history visible (the spawner only sees post-spawn turns; the transcript file has prior + current). Model dropdown (Opus 4.7 / 1M / Sonnet 4.6 / Haiku 4.5 / custom), permission-mode picker with explicit `bypassPermissions` warning, **thinking-effort** field (low / medium / high - wired to `--effort`), cwd autocomplete pre-filled with the dashboard's own cwd plus recent session cwds. Real character-by-character streaming via `--include-partial-messages`, plus a client-side **typewriter smoothing layer** that drips each `text_delta` / `thinking_delta` through `requestAnimationFrame` so even short replies (where claude bundles the whole answer into one or two chunks) appear to type in. The merge code keeps the `_streaming` flag and the delta-accumulated `content` array intact when claude's canonical `assistant` envelope arrives mid-stream, so thinking blocks aren't dropped at completion. WebSocket dispatch wraps each envelope in `flushSync` so React 18's auto-batching doesn't collapse bursts of deltas into a single render. **TUI parity (Tier 1)**: a collapsible **limitations banner** that minimizes to a slim pill (never disappears) explaining what stream-json mode can and can't do vs. the terminal TUI; a **prompt editor with slash-command autocomplete** with tiered scoring (exact name → starts-with → word-boundary → contains → subsequence → description-contains) that lists user / project / plugin commands (executed client-side via template expansion before send) and surfaces built-in CLI commands like `/clear`, `/model`, `/config` with a "CLI only - won't run from here" badge; **`@`-file references** with debounced fuzzy-search across the run's cwd (skipping `node_modules`, `.git`, `dist`, `build`, etc.); a **live context-window / token meter** showing input + output + cache-read tokens and running cost, computed from `stream_event` and `result.usage` envelopes during live streaming and from finalized assistant `usage` blocks (input / output / cache-read / cache-creation) when seeded from a transcript on resume / view / re-attach, so the meter populates immediately instead of sitting at 0/200k. Progress bar goes indigo → amber → red at 80% / 95% of the model's context cap; a **status header** with the active model, effort, permission mode, cwd, session ID, envelope count, and elapsed time. Autocomplete dropdowns open upward so they don't collide with the cwd picker below. Live / Offline indicator next to the title. Same-origin guard on the route prevents browser drive-by spawning. Concurrency is effectively uncapped by default (sanity ceiling of 10000 to prevent fork-bomb footguns from a buggy client; the terminal TUI has no cap and neither do we). Set `RUN_MAX_CONCURRENT` if you want a real ceiling. Spawned sessions fire the same hooks any `claude` process does, so they show up automatically in Sessions / Analytics / Kanban / Workflows - and Sessions / SessionDetail surface a green **▶ Run** badge / banner that links back to the Run page for any session that's currently being driven from there. **Interactive permissions**: an opt-in spawn-form toggle (off by default) arms a PreToolUse gate so every tool call pauses for an Allow/Deny decision in the UI instead of running per the permission mode above; pending requests render inline with tool name + input preview, pulse the Active Runs switcher, and - even when the tab isn't open - fire a web-push notification that deep-links straight to the request |
| **Scheduled & chained prompts**    | Queue a prompt to run **at a time** or **when an existing run completes** - the "run part 4 after part 3 finishes" flow. The **Scheduled** page (`/scheduled`) lists/creates/cancels schedules for both trigger kinds; the Run page has an inline **Queue follow-up** action. `on_run_complete` follow-ups can interpolate `{status}` / `{exitCode}` / `{runId}` from the completed run and can require a clean exit ("only on success"). Fired runs go through the normal spawn path (permission gating, streaming, history all apply); a fired run can itself trigger the next schedule (chaining, depth-guarded, with cancel-cascade). Schedules survive a server restart - pending ones re-arm from SQLite and a missed at-time fires immediately, flagged **late**. Backed by a generic `server/lib/scheduler.js` (a due-callback registry) and a new `onRunStatus` hook on the run-spawner; every fire/failure broadcasts a `schedule_*` WebSocket event and fires the **Scheduled prompts** push category. Fail-safe: a scheduler crash never takes down the server or the watched run |
| **Voice & Siri (Phase D)**         | Talk to Jarvis hands-free from your phone or CarPlay. A Siri Shortcut dictates a request, POSTs it to `POST /api/assistant/ask` over the tailnet, and speaks the `speech` reply. This one endpoint powers Siri / CarPlay / the notes chat / quick actions, returning `{ text, speech }` (a short, markdown-free, number-rounded spoken variant). A deterministic keyword prelude handles high-value intents before the brain - `status` (live runs / waiting agents / active sessions), `kill`/`steer` a run, `note:` (captured to an inbox the Phase G Notes system drains), `run skill` (Phase H) - then falls through to the mini-Jarvis brain (a **stub** until Phase G wires in Gemini/Ollama/Claude; the endpoint contract is stable). Auth is a scoped, revocable **assistant bearer token** generated in **Settings → Voice & Siri**, stored server-side as a SHA-256 hash and shown once - the phone never carries the master dashboard token, and the endpoint is never open (a no-Origin caller must present a token). Rate limited per token. See [`SETUP.md`](./SETUP.md#voice-control-via-siri-shortcuts-phase-d) + [`docs/jarvis-siri-shortcut.md`](./docs/jarvis-siri-shortcut.md) for the Shortcut recipe |
| **Multi-account tracking**         | If you run two Claude accounts via [claude-swap](https://github.com/realiti4/claude-swap), the dashboard tracks both as one unified view - which account is active, each account's window/reset when known, and swap history. **Strictly read-only**: `server/lib/claude-swap.js` observes claude-swap's `~/.claude-swap-backup/autoswitch_state.json` (never the Keychain-stored credentials, never performing a swap itself). An **Accounts** strip under the JARVIS core shows the active-account badge + each other account's reset time, self-hiding entirely when claude-swap isn't detected (zero change for single-account setups). A swap records to a `account_swaps` history table, broadcasts `account_swapped`, and fires the **Account swaps** push category; dashboard-spawned runs are tagged with the account active at spawn time. `GET /api/accounts` exposes the unified state |
| **Multi-provider AI chat (Phase E)** | A dedicated **Chat** page (`/chat`) that talks to multiple AI backends behind one UI - **Gemini** (free-tier API key; also image generation), **Ollama** (your always-on PC's models, reached over Tailscale, discovered live from `/api/tags`), and **Claude** (the local `claude` binary + your existing OAuth, continued across turns via `--resume`). A **GPT slot** is present but inert - ChatGPT free has no API, so it renders as an honest "needs OpenAI API key". Provider/model picker, streaming markdown transcript (Server-Sent Events, reusing the conversation renderer), saved conversations, and inline Gemini image generation stored under the data dir. **All provider calls are server-side; keys never reach the browser bundle** - they live in `server/config/providers.json` (gitignored) edited in **Settings → AI Providers** (env fallbacks: `GEMINI_API_KEY`, `OLLAMA_HOST`). The agentic **Run** page also grows a provider picker: alongside Claude (the default, unchanged, with its permission gate) you can spawn a **Gemini CLI** run (headless, **no permission gate** - the PreToolUse gate is Claude-only), its stream normalized into the same conversation view. See [`SETUP.md`](./SETUP.md#ai-providers--chat-phase-e) |
| **Projects (Phase F)**             | A dashboard-native organizing dimension over sessions, dashboard-spawned runs, and chats - **deliberately separate from Claude.ai's own "Projects" feature**; this has no integration with any Anthropic API. The **Projects** page (`/projects`) is a card grid with per-project rollups (session/run/chat counts, last activity), status filter (active/paused/done), and create; a detail view (`/projects/:id`) edits name/description/status (set status to **done** to archive - there's no separate archive endpoint), manages the repo paths used for auto-association, and shows an aggregated recent-activity feed. **Auto-association**: a project may span multiple repos, so paths live in a one-to-many `project_paths` table; hook-ingested sessions and dashboard-spawned runs are both tagged by matching their `cwd` against the longest matching `project_paths` prefix (path-boundary aware) - `POST /api/run` also accepts an optional explicit `projectId` that wins over the cwd guess. Adding a repo path retroactively backfills any existing session/run whose cwd matches and has no project yet. Chats have no cwd, so `chats.project_id` is only ever set explicitly (`POST`/`PATCH /api/chat/chats/:id`). Deleting a project **un-tags, never deletes** the sessions/runs/chats it grouped - the project is an organizing label, not the system of record. Schema is additive: nullable `project_id` on `sessions` / `dashboard_runs` / `chats` |
| **Notes + mini-Jarvis brain (Phase G)** | Notes are **plain markdown files on disk** (default `~/JarvisNotes`, editable in Obsidian or anywhere) with YAML frontmatter; SQLite is a rebuildable **index + FTS5 search** that an `fs.watch` watcher keeps in sync, so an external edit shows up live (broadcasts `note_changed`). The **Notes** page (`/notes`) has a pinned quick-capture box, full-text search + tag filters, and a list↔editor split with markdown preview. **Brain-dump flow**: drop a messy thought and the **mini-Jarvis brain** - a tiered router (simple→Ollama, standard→Gemini, complex→`claude -p`) with a fallback chain and a `brain_calls` log - reformats it into a structured note (title, cleaned body, extracted todos, tags), shown as a raw↔formatted confirm before saving; it degrades to a deterministic pass-through when no provider is configured. The Phase-D voice/chat "note: …" capture inbox drains through the same path. **Project pulse**: a deterministic daily task (armed on the shared Phase-L scheduler, not a second scheduler) computes each project's working/neglected/completed state from last session/run/chat/**note** activity + open `- [ ]` todos, rendered on the Projects grid ("Neglected: X - no activity in N days"), the project detail, and `GET /api/projects/pulse`. Notes dir is set from the Notes page or the `JARVIS_NOTES_DIR` env var |
| **Knowledge Vault - the second brain (Phase S)** | The notes tree becomes a typed, traversable **graph**: PARA-style folders (`inbox/ projects/ people/ reference/ daily/ agent/`), `[[wikilink]]` + frontmatter-relation edges in an additive `vault_edges` index (an unresolved link resolves the moment its target file appears; titles and slugged filenames both answer), and a **`/vault` graph-brain view** - a force-directed canvas of colored type clusters (Obsidian graph-view style) with hover neighborhoods, a node side panel with backlinks, focus mode (`?focus=<id>` deep link), search dimming, and pinch/zoom on mobile. Auto-population v1: **run summaries** (opt-in per project in **Settings → Knowledge Vault**; brain-composed on run completion, wikilinked to a `projects/` hub page, factual fallback when no provider is configured) and **chat save-to-vault** (bookmark any assistant reply, or condense the whole conversation via the Chat header's Vault button). Agents traverse the same memory: six `vault_*` assistant actions (all risk `safe`) and `dashboard_vault_*` MCP tools; agent writes are confined to `inbox/` + `agent/` and **never overwrite** a human note. Everything stays markdown-on-disk in the Phase-G notes dir - Obsidian keeps working on the same folder. Plan: [`PLAN-jarvis-vault.md`](./PLAN-jarvis-vault.md) |
| **Vault Brain - the engine (Phase T)** | The vault builds its own connections. A manual **Run engine** pass (Vault page button; no cron) extracts entities (person/project/organization/topic/place/event/technology) from new/changed notes via the brain router, **promotes an entity to a real node file on its 2nd mention across distinct notes** (immediately if a note by that name already exists; aliases match too), and writes `[[wikilinks]]` into every mentioning note inside one engine-owned `<!-- jarvis:links -->` block - raw-text swapped, idempotent, human prose never touched, first mentions linked retroactively. While it runs the graph **visibly thinks**: scanned notes fire expanding rings, synapses flicker, newborn nodes flash halos, new edges grow in bright from source to target (reduced-motion aware). Plus **Graphify codegraphs**: opt a project in (Settings → Knowledge Vault) and build a knowledge graph of its repo (`graphify extract --code-only`, community naming via your Claude session - no API key anywhere) exported into `projects/<slug>/codegraph/` for Obsidian, with one overview node stitched into the dashboard graph. Node panel gains an **Open in Obsidian** deep link. Plan: [`PLAN-vault-brain.md`](./PLAN-vault-brain.md) |
| **Skills - tap-to-run automations (Phase H)** | Skills are **markdown files with YAML frontmatter** on disk (default `~/JarvisSkills`, override with the `JARVIS_SKILLS_DIR` env var or from the Skills page - same file-first philosophy as Notes) - no bespoke form builder, no second scheduler. A skill is a straight pipeline of typed steps: `shell` (a local command), `agent` (spawn a headless Claude/Gemini run), `brain` (one mini-Jarvis call), `notify` (a push), and `phone` (a push that hands off to an iOS Shortcut). Step outputs interpolate into later steps as `{stepN_output}` / `{<type>_output}`. A `confirm` level gates who may trigger it: **none** (tap, voice, phone, or its own cron `schedule`), **tap** (a human taps Run), or **typed** (retype the skill's name) - voice/phone/schedule triggers can **only** ever fire a `confirm: none` skill, enforced server-side, not just in the UI. The **Skills** page (`/skills`) is a mobile-first tap-target grid with a run sheet (params + confirmation), live per-step progress over WebSocket, run history, and a raw-markdown editor for creating/editing skill files. A `schedule` (5-field cron) fires a skill automatically via a tick on the **shared** Phase-L scheduler (`registerRecurringTask`, not a second one). Voice: "run skill `<name>`" (Phase D) now actually runs a `confirm: none` skill instead of the earlier stub, and gives an honest spoken reason for anything requiring confirmation. Execution history lives in a `skill_runs` table (per-step JSON progress); the library itself has no SQLite index - it's read straight off disk. See [`docs/skills/`](./docs/skills/) for three seed examples (a scheduled daily briefing, a typed-confirm cleanup, an agent-spawning skill) |
| **GitHub dev-workflow panel (Phase I)** | A work panel at `/github` that surfaces, across a configured list of repos, the **latest activity** (the most recent commit or merge on each repo's default branch, shown as branch + message - never the raw SHA; a GitHub-style merge commit is rendered "Merged PR #N: \<title\>" with the source branch), PRs **awaiting your review**, **your own open PRs** (with CI/check status), and **recent open issues**, plus a home command-bridge widget that self-hides until configured ("2 PRs need review · 1 CI red", tappable to the page). Data comes from the locally-authenticated **`gh` CLI** (recommended - no secret stored; provides per-PR CI rollup) or a server-side **Personal Access Token** (REST mode; portable to hosts without `gh`, but CI status is `gh`-only). All calls are server-side; the token is never shipped to the client (only a `hasPat` boolean). The server polls on the shared Phase-L scheduler and caches a single-row snapshot in `github_cache`, broadcasting `github_updated` only on a real delta; a newly-requested review or newly-red check fires the `github` push category. Watched repos + PAT + poll cadence are edited inline on the GitHub page (`GET/PUT /api/github/config`, PAT redacted). See [`SETUP.md`](./SETUP.md#github-dev-workflow-panel-phase-i) for `gh auth login` / PAT setup |
| **Monday.com panel (Phase AD)** | A work panel at `/monday` - a deliberate 1:1 clone of the GitHub panel's architecture - showing, via the **Monday GraphQL API** with a personal token (works on the free plan), the items **assigned to you** (grouped by board, detected through people columns), items **due today / overdue** (date columns, computed against the server's local date), and **recent updates**, plus a self-hiding home widget ("2 overdue · 3 due today"). All calls are server-side; the token lives in gitignored `server/config/monday.json` (or `MONDAY_TOKEN`) and is never shipped to the client (only `hasToken`). The server polls on the shared Phase-L scheduler, caches a single-row `monday_cache` snapshot, broadcasts `monday_updated` only on a real delta, and fires the `monday` push category when an item is newly assigned to you or newly due today. **Write-back is deliberately minimal**: "mark done" sets the item's status column to a configurable done label (`change_simple_column_value`) - that's the one mutation, used by the Today board. Token + done label + poll cadence are edited inline on the page (`GET/PUT /api/monday/config`, token redacted). See [`SETUP.md`](./SETUP.md#mondaycom-panel-phase-ad) |
| **Today board (Phase AC)** | The daily glance surface at `/today` - and the **first tab on the phone** (replacing Home as the mobile landing; desktop home unchanged). `GET /api/today` aggregates server-side with **no new storage**: open `- [ ]` todos across notes (the markdown files ARE the todo store), Monday items due/overdue (from the AD cache), today's scheduled prompts (Phase L), agents waiting on you, and today's runs - laid out as **Do today / In flight / Done today** lanes. Checking a note todo rewrites that `- [ ]` → `- [x]` line **in the markdown file itself** via the existing notes write path (Obsidian sees it; the watcher rebroadcasts it); a stale line ref is re-found by exact text. Checking a Monday item calls the AD write-back. Agent/schedule rows deep-link - they don't complete from the board. The morning briefing leads with a one-sentence "top of today" composed from the same aggregation |
| **Proactive Jarvis - briefings & nudges (Phase J)** | Jarvis reaches out first. A **Briefings** page (`/briefings`) composes a **morning briefing** and an **end-of-day summary** from state the earlier phases already produce - project pulse (Phase G2), the GitHub overview (Phase I), dashboard-run activity since midnight, and agents waiting on you. Context assembly is deterministic; the prose is then written by the brain (standard tier, **in Jarvis's persona**) when a provider is configured, or composed deterministically from the same facts when none is - so nothing is ever invented and there's always something usable. Each briefing is filed as a markdown note (`source: briefing`), pushed under the `briefings` category, and returned with a short Siri-readable `speech` variant. Two scheduled ticks (morning/evening, times set on the page) run on the **shared** Phase-L scheduler; "Run now" and the "Hey Siri, Jarvis, morning briefing" voice intent trigger it on demand. **Deterministic nudges** (never model-composed): a failed run → a `run_completions` push; an agent waiting past your threshold → a `waiting_agents` push (once per waiting spell); neglected projects are surfaced in briefings, not instant-pushed. All nudges respect the notification-category toggles. **JARVIS personality**: a single toggle gives the assistant/chat replies, the briefings, and the nudge copy a dry, formal-but-warm butler voice that addresses you as "sir" (`JARVIS_PERSONA`, on by default; off = neutral phrasing). History + all Phase-J config live behind `GET/POST /api/briefings` + `GET/PUT /api/briefings/config` |
| **Subscriptions & finance tracker (Phase AE)** | A **Finance** page (`/finance`) tracking recurring subscriptions - **manual entries only, no bank/Plaid integrations by design**. The server owns the date math: `next_renewal` defaults to one cadence from today and past-due dates roll forward (month-end clamped) on save and on a once-daily tick. A summary header shows the **per-currency monthly burn** (yearly ÷ 12, custom cadences normalized; currencies never FX-mixed), the yearly projection, and the next renewal; a self-hiding home widget mirrors it. **Paste-to-parse**: paste a bank-statement / receipts blob and the brain (standard tier, deterministic heuristic fallback) extracts candidate subscriptions for confirm-before-save - the same raw→formatted confirm UX as the notes dump flow. Renewals ≤2 days out fire a **`finance`-category push** (deduped per renewal) from the shared Phase-L scheduler and are mentioned in briefings. `GET/POST /api/subscriptions`, `PUT/DELETE /api/subscriptions/:id`, `GET /api/subscriptions/summary`, `POST /api/subscriptions/parse` |
| **Declutter + native handoff (Phase AF)** | Ambient surfaces go quiet by default. A **feed verbosity** setting (`agent` default / `tool`, stored server-side in `app_settings`, toggled contextually on the surfaces themselves) folds consecutive tool envelopes on the home Operations feed and the Activity Feed behind **expandable agent rows**, and hides the noisier home instruments (total-sessions, events/min); Run/SessionDetail stay fully verbose for debugging. The home-rolled Phase-Z remote-screen surfaces (`/browse`, `/computer-use` and their assistant actions) are **retired in favour of native tools** - RustDesk over the tailnet for live mirroring (see [`SETUP.md`](./SETUP.md#remote-screen-optional), plus a mobile-only `rustdesk://` deep-link button on home) and ChatGPT Work for agentic computer use - and parked for one release behind `LEGACY_SURFACES=1` |
...
| `CLAUDE_SWAP_BACKUP_DIR` | `~/.claude-swap-backup` | Directory the read-only multi-account observer watches for claude-swap state (Phase K). Override for a non-default install; the observer stays inert when the state file is absent |
| `CLAUDE_SWAP_POLL_MS`   | `60000`       | Safety-net poll interval for the claude-swap state file. `0` disables the poll but keeps the `fs.watch` |
...
| `GEMINI_API_KEY`        | _(unset)_     | Zero-config fallback for the Chat harness's Gemini key (Phase E). Fills `gemini.apiKey` only when `server/config/providers.json` leaves it empty; a value saved in Settings → AI Providers always wins. Server-side only |
| `OLLAMA_HOST`           | `http://localhost:11434` | Fallback for the Ollama host (Phase E) - point it at your always-on PC's tailnet name. Overridden by a value saved in Settings |
| `OPENAI_API_KEY`        | _(unset)_     | Fallback for the (currently inert) GPT slot's key (Phase E) |
| `PROVIDERS_CONFIG_PATH` | `server/config/providers.json` | Override the provider-config file location (Phase E; tests / custom setups) |
| `GEMINI_CLI_COMMAND`    | `gemini`      | Binary for the Gemini CLI agentic backend (Phase E, §E2) |
| `GEMINI_CLI_ARGS`       | _(unset)_     | Extra space-separated flags for the Gemini CLI backend, for host-specific tuning |
...
| `POST`   | `/api/run`                        | Spawn a new run. Body: `{ prompt, mode: "headless"\|"conversation", cwd?, model?, permissionMode?, permissionUx?, resumeSessionId?, effort? }`. Headless puts prompt in argv via `-p` and closes stdin. Conversation pipes the prompt over stdin as a stream-json envelope and keeps stdin open for follow-ups. `resumeSessionId` (conversation only) adds `--resume <id>`; when set, `prompt` may be empty - the spawner skips the initial stdin write and `claude` idles on the resumed conversation until the user posts a follow-up via `POST /api/run/:id/message`. `effort` (`low` / `medium` / `high`) maps to `--effort`. `permissionUx: "interactive"` (opt-in only) arms the PreToolUse permission gate - see below. The spawner always passes `--output-format stream-json --verbose --include-partial-messages` so the UI can render character-by-character deltas. Concurrency is effectively uncapped (default ceiling 10000 - override with `RUN_MAX_CONCURRENT`) |
| `GET`    | `/api/run/:id`                    | Current handle state. `?envelopes=1` includes the in-memory envelope log so the UI can replay history when re-attaching. Always includes `pendingPermissions` (`[]` unless the gate is armed) |
...
| `POST`   | `/api/run/:id/permission/request` | Opened by the PreToolUse gate hook (`scripts/permission-gate.js`), not the UI. Body: `{ toolUseId, toolName?, toolInput? }` |
| `GET`/`POST` `/api/run/:id/permission/request/:requestId` | Hook short-polls the `GET` for a decision; the dashboard UI posts the decision via `POST` - Body: `{ decision: "allow"\|"deny", reason? }` |

Output streams over the existing dashboard WebSocket as five message types: `run_stream` (parsed stream-json envelope, including `stream_event` deltas from `--include-partial-messages`), `run_status` (status transitions), `run_input_ack` (stdin write confirmed), and `permission_request` / `permission_resolved` (interactive-permission-gate lifecycle - payload `{ id: <runId>, request: { requestId, toolName, toolInput, status, decision, reason, openedAt, resolvedAt } }`). The Config Explorer page subscribes to a sixth message - `cc_config_changed` - broadcast by `server/lib/cc-watcher.js` (via `fs.watch` on `~/.claude/`) and by `routes/cc-config.js` after every successful PUT/DELETE, with payload `{ source: "dashboard"|"fs", action?, scope?, type?, name?, paths? }`. The Sessions list and SessionDetail page poll `/api/run` (and listen for `run_status`) to badge any session currently being driven by an in-flight Run with a clickable **▶ Run** indicator that links back to `/run`.

**Interactive permissions**: toggle "Interactive permissions" on the Run page's spawn form (unavailable under `bypassPermissions`, which never triggers the gate) to send `permissionUx: "interactive"`. Every tool call then pauses; the pending request renders on the Run page (`client/src/components/PermissionRequests.tsx`) with tool name + input preview and Allow/Deny buttons, seeded from `GET /api/run/:id/permissions` on attach/reconnect and kept live via the WS broadcasts above (degrading to short-polling that same endpoint while the socket is disconnected). A stale request the hook never resolved auto-denies after an 11-minute safety-net TTL. A new request also pulses the Active Runs switcher/list and fires a web-push notification deep-linking to `/run?runId=<id>#permission-<requestId>`, so a pending decision reaches you even with the tab closed.

### Accounts & Schedules (`/api/accounts`, `/api/schedules`)

| Method   | Endpoint                | Description                                                                              |
| -------- | ----------------------- | --------------------------------------------------------------------------------------- |
| `GET`    | `/api/accounts`         | Unified read-only multi-account (claude-swap) state: `{ present, activeAccountId, accounts[], swaps[] }`. `present:false` for single-account setups |
| `GET`    | `/api/schedules`        | List scheduled/chained prompts (`?status=pending\|fired\|cancelled\|failed`) → `{ items[], maxChainDepth }` |
| `POST`   | `/api/schedules`        | Create. Body: `{ prompt, targetKind: "new_run"\|"session_message", targetOpts?, triggerKind: "at"\|"on_run_complete", fireAt?, triggerRunId?, statusFilter?: "any"\|"success", label? }` |
| `GET`    | `/api/schedules/:id`    | One schedule                                                                             |
| `PATCH`  | `/api/schedules/:id`    | Edit a pending schedule. Body: `{ label?, prompt?, fireAt?, statusFilter? }`             |
| `DELETE` | `/api/schedules/:id`    | Cancel a pending schedule; `?cascade=1` also cancels its chained dependents              |
| `POST`   | `/api/assistant/ask`    | Voice/assistant (Phase D). Body: `{ text, source?, conversationId? }` → `{ text, speech, intent, ... }`. Requires an assistant bearer token (`Authorization: Bearer <token>`); exempt from `DASHBOARD_TOKEN` but never open. Rate limited per token |
| `GET`    | `/api/assistant/tokens` | List assistant tokens (hash-only, no secret). Admin route: behind `DASHBOARD_TOKEN` + a loopback same-origin guard |
| `POST`   | `/api/assistant/tokens` | Generate a token (secret returned once). Body: `{ label? }`                              |
| `DELETE` | `/api/assistant/tokens/:id` | Revoke a token                                                                       |

Schedules broadcast `schedule_created` / `schedule_updated` / `schedule_cancelled` / `schedule_fired` / `schedule_failed`; a claude-swap transition broadcasts `account_swapped`. Both fire optional push categories (**Scheduled prompts**, **Account swaps**). See [`docs/API.md`](./docs/API.md) for payload shapes.

### Notes & Project Pulse (`/api/notes`, `/api/projects/pulse`) - Phase G

| Method   | Endpoint                     | Description                                                                              |
| -------- | ---------------------------- | --------------------------------------------------------------------------------------- |
| `GET`    | `/api/notes`                 | List/search notes. `?q=` FTS5 full-text, `?tag=`, `?project=` filters → `{ items[] }` (index metadata, no body) |
| `GET`    | `/api/notes/:id`             | One note **with** its markdown body                                                     |
| `POST`   | `/api/notes`                 | Create. Body: `{ title?, body?, tags?, projectId? }` → writes a markdown file + indexes it |
| `PUT`    | `/api/notes/:id`             | Update `{ title?, body?, tags?, projectId? }` (rewrites the file, preserves id/created/original) |
| `DELETE` | `/api/notes/:id`             | Delete the note (removes the file + index row)                                          |
| `GET`    | `/api/notes/tags`            | Distinct tags with counts                                                               |
| `GET`/`PUT` | `/api/notes/config`       | Get/set the notes directory (`{ dir, default }`)                                        |
| `POST`   | `/api/notes/dump`            | Brain-dump reformat. Body: `{ text, save?, projectId?, source? }` → `{ raw, formatted, provider, title, body, tags, todos, note? }`. `save:false` previews (raw↔formatted confirm); `save:true` files a `source:dump` note preserving the original |
| `GET`    | `/api/notes/captures`        | Pending voice/chat "note: …" capture inbox                                              |
| `POST`   | `/api/notes/captures/:id/file` / `.../discard` | File a capture as a note (through the brain), or drop it                |
| `GET`    | `/api/projects/pulse`        | Current per-project pulse rows → `{ items[], neglectDays }`                             |
| `POST`   | `/api/projects/pulse/recompute` | Force an immediate pulse recompute (also runs daily on the shared scheduler)         |

Notes broadcast a `note_changed` WebSocket message whenever the on-disk watcher reindexes (an edit made in Obsidian, by an agent, or by hand). The mini-Jarvis brain router (`server/lib/brain/`) logs every routing decision to the `brain_calls` table.

### Knowledge Vault (`/api/vault`) - Phase S

| Method   | Endpoint                       | Description                                                                              |
| -------- | ------------------------------ | ---------------------------------------------------------------------------------------- |
| `GET`    | `/api/vault/graph`             | The whole graph → `{ nodes[] (id, title, type, tags, projectId, updatedAt), edges[] (src, dst, type) }` (resolved edges only) |
| `GET`    | `/api/vault/node/:id`          | One node: the note + `nodeType`, `outgoing[]` links (resolved + unresolved) and `backlinks[]` |
| `GET`    | `/api/vault/path?from=&to=`    | Shortest wikilink chain between two nodes (undirected BFS) → `{ path[] \| null }`        |
| `GET`/`PUT` | `/api/vault/summary-projects` | Run-summary opt-in project ids; a `PUT` materializes a `projects/` hub page per opted-in project |
| `POST`   | `/api/vault/save-chat`         | `{ chatId, mode: "message"\|"summary", messageId? }` → files a chat reply / brain-condensed summary under `agent/chats/` |
| `POST`   | `/api/vault/write`             | Guardrailed write: **only** `inbox/` or `agent/…`, always a fresh file (403 otherwise)   |
| `POST`   | `/api/vault/engine/run`        | Entity-engine pass (Phase T): extract entities from changed notes, promote on 2nd mention, write auto-links. Manual only; `409` while running; progress streams as `vault_engine` WS events |
| `GET`    | `/api/vault/engine/status`     | `{ running, lastRun, totalEntities, promotedEntities }`                                  |
| `GET`/`PUT` | `/api/vault/graphify-projects` | Graphify codegraph opt-in project ids                                                 |
| `POST`   | `/api/vault/graphify/run`      | `{ projectId }` → build + export a repo codegraph into the vault (`202`; progress via `vault_graphify` WS events; `GRAPHIFY_BIN` env overrides the CLI path) |
| `GET`    | `/api/vault/graphify/status`   | `{ running[], projects{<id>: {lastRun, ok, error, nodes, edges}} }`                      |

Vault edges ride the notes pipeline (same watcher, same `note_changed` broadcast); the `/vault` page re-fetches on it, preserving graph layout positions.

### Skills (`/api/skills`) - Phase H

| Method   | Endpoint                     | Description                                                                              |
| -------- | ---------------------------- | ----------------------------------------------------------------------------------------- |
| `GET`    | `/api/skills`                | List every skill definition (parsed from `~/JarvisSkills`) → `{ items[] }`               |
| `GET`    | `/api/skills/:id`            | One skill, **with** its raw markdown+frontmatter file contents                            |
| `POST`   | `/api/skills`                | Create from raw markdown+frontmatter. Body: `{ raw }`                                     |
| `PUT`    | `/api/skills/:id`            | Overwrite a skill's file contents. Body: `{ raw }`                                        |
| `DELETE` | `/api/skills/:id`            | Delete the skill (removes the file)                                                       |
| `GET`/`PUT` | `/api/skills/config`      | Get/set the skills directory (`{ dir, default }`)                                         |
| `POST`   | `/api/skills/:id/run`        | Execute. Body: `{ params?, confirmText? }` (`confirmText` required for `confirm: typed`) → `{ run }`; 409 `ECONFIRM` if confirmation is missing/wrong or the skill needs more than a tap |
| `GET`    | `/api/skills/runs`           | Run history (`?skillId=`) → `{ items[] }`                                                 |
| `GET`    | `/api/skills/runs/:id`       | One run, with live per-step progress                                                      |
| `POST`   | `/api/skills/runs/:id/cancel` | Cancel a running skill (kills an in-flight `shell` step immediately; takes effect before the next step otherwise) |

Skill runs broadcast `skill_run_started` / `skill_run_step` / `skill_run_finished` / `skill_run_failed`; an on-disk skill file change broadcasts `skill_changed`. Both fire the **Skill runs** push category. Like `/api/run` and `/api/schedules`, this router sits behind the loopback-Origin guard since running a skill can spawn shell/agent processes. See [`docs/API.md`](./docs/API.md) for payload shapes and [`docs/skills/`](./docs/skills/) for example skill files.

### GitHub (`/api/github`) - Phase I

| Method   | Endpoint                | Purpose                                                                                     |
|----------|-------------------------|---------------------------------------------------------------------------------------------|
| `GET`    | `/api/github`           | The cached cross-repo overview → `{ overview, fetchedAt, error, mode, configured }`          |
| `POST`   | `/api/github/refresh`   | Force a live poll now (spawns `gh` / hits REST); broadcasts `github_updated` on a delta      |
| `GET`    | `/api/github/config`    | Redacted config → `{ config: { enabled, hasPat, repos, pollMinutes } }` (PAT never returned) |
| `PUT`    | `/api/github/config`    | Update config. Body: `{ enabled?, pat?, repos?, pollMinutes? }` (PAT stays server-side)      |

The overview carries `reviewRequested` / `mine` (PRs, each with a `ci` state of `success`/`failure`/`pending`/`none`/`unknown`) and `issues`, plus `counts`. A fresh poll broadcasts `github_updated` only when the fingerprint changes, and a newly-requested review or newly-red check fires the **GitHub** push category. Like `/api/run`, this router sits behind the loopback-Origin guard since `/refresh` spawns `gh` and `/config` writes a secret. See [`docs/API.md`](./docs/API.md) for payload shapes.

### Briefings (`/api/briefings`) - Phase J

| Method   | Endpoint                  | Purpose                                                                                          |
|----------|---------------------------|-------------------------------------------------------------------------------------------------|
| `GET`    | `/api/briefings`          | Recent briefings + latest per kind → `{ items, latest: { morning, evening } }`                   |
...
A briefing composes from project pulse + GitHub + run/agent activity, is filed as a note, pushed under the `briefings` category, and broadcast as `briefing_created`. Scheduled morning/evening ticks run on the shared Phase-L scheduler; the "morning briefing" voice intent (`POST /api/assistant/ask`) triggers the same path. Nudges (run-failed, waiting-agent) are deterministic and gated by the notification-category toggles. `persona` toggles the JARVIS voice across the assistant, briefings, and nudges. This router sits behind the loopback-Origin guard (composing writes notes + fires pushes). See [`docs/API.md`](./docs/API.md).

### Subscriptions (`/api/subscriptions`) - Phase AE

| Method   | Endpoint                     | Purpose                                                                                            |
|----------|------------------------------|----------------------------------------------------------------------------------------------------|
| `GET`    | `/api/subscriptions`         | All subscriptions (active first, soonest renewal first) → `{ subscriptions }`                      |
| `POST`   | `/api/subscriptions`         | Create. Body: `{ name, amount, currency?, cadence?, cadence_days?, next_renewal?, ... }` → `201`   |
| `PUT`    | `/api/subscriptions/:id`     | Partial update (any create field + `active`) → `{ subscription }`                                  |
| `DELETE` | `/api/subscriptions/:id`     | Delete → `{ ok }`                                                                                  |
| `GET`    | `/api/subscriptions/summary` | Per-currency monthly burn + yearly projection + next-7-day renewals → `{ by_currency, upcoming }`  |
| `POST`   | `/api/subscriptions/parse`   | `{ text }` → candidate subscriptions (brain-extracted, heuristic fallback); nothing is persisted   |

The server owns the date math (past-due `next_renewal` rolls forward, month-end clamped, on save and on a once-daily tick), fires a `finance`-category push for renewals ≤2 days out (deduped per renewal), and broadcasts `subscriptions_updated` on any change. See [`docs/API.md`](./docs/API.md).
...
- **Service Worker:** A dedicated worker (`client/public/sw.js`) handles incoming `push` events and displays notifications with `silent: false` to ensure audio playback on macOS. `notificationclick` navigates to a deep link when the payload carries one (`data.url`) instead of just focusing the dashboard.
- **Subscriptions:** Browser-specific endpoints are stored in the `push_subscriptions` table in SQLite.
- **Deep links:** `sendPushToAll(db, title, body, url?, category?)` accepts an optional `url`, carried as `data.url`. The Run page's interactive permission gate is the first producer of these - a new pending request pushes a notification linking straight to `/run?runId=<id>#permission-<requestId>`.
- **Delivery categories:** server-originated pushes are tagged with a category (`permission_requests` today; `run_completions` / `waiting_agents` / `briefings` reserved for later phases). **Settings → Notifications → Push categories** exposes a per-category on/off switch stored **server-side** (`notification_prefs` table via `GET`/`PUT /api/push/categories`), so muting a category silences it for every subscribed device - distinct from the per-browser event toggles above. A muted category makes `sendPushToAll` a no-op; a category with no stored preference defaults to on, and any lookup error fails open so a permission request is never silently dropped.
...
## Mini JARVIS - Floating Companion

**Mini JARVIS** is a floating pocket arc-reactor companion pinned to the bottom-right corner of every page in the dashboard. Always present, it turns the live session stream into an at-a-glance, reactive orb you can also talk to. (Internally the module keeps its original codename, `Tabby`.)

<p align="center">
  <img src="images/tabby.png" alt="Mini JARVIS, the reactive dashboard companion, shown in various moods and with a speech bubble" width="100%">
</p>

### Reactive orb

Mini JARVIS is an SVG arc-reactor orb whose glowing iris tracks your cursor, with **eight moods** derived from the live session stream, each with its own animation:

| Mood | When | Animation |
| --- | --- | --- |
| `idle` | Nothing notable happening | Rotating tick rings, breathing nucleus |
| `watching` | Sessions are active | Iris follows the cursor |
| `happy` | A session or run finished cleanly | Sparkle + brighter breathing |
| `worried` | Something looks off | Warning arc over the crown |
| `stuck` | A session appears blocked | Alert "!" + rings freeze |
| `thinking` | An agent is mid-work | Rings spin faster |
| `sleeping` | Quiet for a while | Lid closes, `zzz`, dimmed core |
| `disconnected` | WebSocket is down | Desaturated, rings halt |

### Speech bubbles

Mini JARVIS auto-surfaces short quips on notable events (session started/finished, errors, run completed). Bubbles are **throttled and coalesced** so a burst of events never spams the screen, they use `aria-live` for screen readers, and they can be **muted** from the panel.

### Panel

Open the panel by clicking the orb or pressing **⌘B / Ctrl+B** (Esc closes). It shows:
...
<p align="center">
  <img src="images/windows_app.png" alt="Claude Code Monitor running as a native Windows desktop app, showing the Activity Feed with the Windows window menu bar and Mini JARVIS panel" width="100%">
  <br>
