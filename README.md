...
- **Deep links:** `sendPushToAll(db, title, body, url?)` accepts an optional `url`, carried as `data.url`. The Run page's interactive permission gate is the first producer of these — a new pending request pushes a notification linking straight to `/run?runId=<id>#permission-<requestId>` (not yet gated by a per-event toggle above; that lands with the rest of the Run-page push categories in a later pass).
